  ...
  return (
    <div className="fw-study">
      ...
      <section id="result" className="fw-section">
        ...
        <Heading n="기술적 한계와 반성" title="라우팅 제약 분리와 네트워크 프로토콜 선택에 대한 회고" />
        <Facts
          items={[
            [
              '라우팅 제약 미분리 (Hard vs Soft)',
              '필수 조건(Context 한도·예산)과 단순 선호도를 구분 없이 합산해 결격 모델이 추천될 위험 확인 → Hard Filter(자격 검증) 후 Soft Scoring(가중 합산)하는 단계별 분리 필요',
            ],
            [
              '프로토콜 오버엔지니어링 (WebSocket vs SSE)',
              '단방향 토큰 스트리밍에 양방향 WebSocket을 도입해 세션 관리 복잡도 초래 → 단방향 전송에는 REST + SSE가 훨씬 단순하고 견고한 아키텍처임을 확인',
            ],
          ]}
        />
        <p className="fw-note">
