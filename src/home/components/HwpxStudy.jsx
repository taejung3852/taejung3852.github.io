...
export default function HwpxStudy(){
 ...
 <section id="problem" className="fw-section">
  ...
  <div className="hx-pairs">
   <div><h3>문항은 XML에 다 있습니다</h3><p>HWPX는 XML을 압축한 형식이라, 양식에 어떤 문항이 있는지는 그대로 읽힙니다. ‘성 Surname’, ‘생년월일 Date of Birth’ 같은 문구가 어느 셀에 있는지도 정확히 알 수 있습니다.</p></div>
   <div><h3>답을 쓸 칸은 적혀 있지 않습니다</h3><p>문제는 그다음입니다. 그 문항의 <strong>답을 어느 칸에 써야 하는지</strong>는 파일 어디에도 없습니다. 문항과 답 칸을 이어 주는 정보가 XML에 존재하지 않습니다. 사람이 문서를 볼 때의 배치로만 드러납니다.</p></div>
   <div><h3>같은 문구의 칸이 네 곳입니다</h3><p>대표로 사용한 통합신청서에는 전화·연락처를 적는 칸이 여섯 곳 있습니다. 그중 네 곳은 <strong>‘전화번호 Phone No.’라는 완전히 같은 문구</strong>입니다. 본국 주소, 학교, 원 근무처, 예정 근무처에 각각 하나씩 붙어 있어, 문구만으로는 구분할 수 없습니다.</p></div>
   <div><h3>한 값이 여러 칸으로 흩어집니다</h3><p>생년월일은 년·월·일 세 칸에, 외국인등록번호는 열세 칸에 한 자씩 들어갑니다. 성과 명도 각각 다른 칸입니다. 값 하나를 어디에 쓸지가 아니라, 어떻게 쪼개 넣을지까지 정해야 합니다.</p></div>
  </div>
